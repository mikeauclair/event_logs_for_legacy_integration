...
import { describe, beforeEach, it, xit } from "mocha";
...
import { handleInput, compactLog } from "./handler";
...
describe("original events", () => {
  ...
  it("should compact events", () => {
    let newEvents = compactLog(events);
    expect(newEvents.map(e => e[0])).to.deep.equal(["setGroup", "setGroup", "setGroupLimit", "setGroupLimit", "setVehicleGroupAssignment", "setVehicleGroupAssignment"]);
  });

  it("should compact recurring events", () => {
    let dupeEvents: event[] = [
      ["setGroupLimit", 1, 25000], ["setGroupLimit", 1, 32000]
    ];
    let newEvents = compactLog([ ...events, ...dupeEvents ]);
    expect(newEvents.map(e => e[0])).to.deep.equal(["setGroup", "setGroup", "setGroupLimit", "setVehicleGroupAssignment", "setVehicleGroupAssignment", "setGroupLimit"]);
  });
  ...
  it("should have expected results for legacyInputInitial", () => {
    ...
    expect(response.outputs[response.outputs.length-1]).to.deep.equal({
      groups: [
        {
          id: 1,
          vehicles: [
  ...
  describe('missingPersonOne', () => {
    ...
    it('should compact', () => {
      let response = handleInput(legacyInputMissingPersonOne, events);
      let newEvents = compactLog(response.events);
      expect(newEvents.map(e => e[0])).to.deep.equal(["setGroup", "setGroup", "setGroupLimit", "setGroupLimit", "setVehicleGroupAssignment", "setVehicleGroupAssignment"]);
    });
  });
