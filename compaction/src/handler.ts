...
const compactionKeyForEvent = (event: event): string => {
  ...
  switch (eventType) {
    ...
    case "setExclusion":
      return eventType + event[EventIndex.Target] + event[EventIndex.Value];
    case "removeExclusion":
      return "setExclusion" + event[EventIndex.Target] + event[EventIndex.Value];
    case "noop":
      return "";
    default:
      assertNever(eventType, "parameter");
...
export const compactLog = (events: event[]): event[] => {
  ...
  let results = events.reduceRight((accum, event) => {
    ...
    let key = compactionKeyForEvent(event);
    if (seen.has(key)) {
      return accum;
    }
    seen.add(key);
    // We know that the addition will be cleared out, so we can noop this
    if (event[EventIndex.EventCommand] == "removeExclusion") {
      return accum;
    };
